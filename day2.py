puzzle_input = '''
'''

steps = puzzle_input.splitlines()


def normal():
    # 1 2 3
    # 4 5 6
    # 7 8 9

    keypad = {}
    for number in xrange(1, 10):
        keypad[number] = {
            'U': number - 3 if number - 3 >= 1 else number,
            'D': number + 3 if number + 3 <= 9 else number,
            'L': number - 1 if number % 3 is not 1 else number,
            'R': number + 1 if number % 3 is not 0 else number
        }

    # start at 5
    current = 5

    code = []
    for line in steps:
        for step in line:
            current = keypad[current][step]
        code.append(current)

    print "normal: ", ''.join([str(v) for v in code])


def alt():
    #     1
    #   2 3 4
    # 5 6 7 8 9
    #   A B C
    #     D
    alt_keypad = {
        1: {'U': 1, 'D': 3, 'L': 1, 'R': 1},
        2: {'U': 2, 'D': 6, 'L': 2, 'R': 3},
        3: {'U': 1, 'D': 7, 'L': 2, 'R': 4},
        4: {'U': 4, 'D': 8, 'L': 3, 'R': 4},
        5: {'U': 5, 'D': 5, 'L': 5, 'R': 6},
        6: {'U': 2, 'D': 'A', 'L': 5, 'R': 7},
        7: {'U': 3, 'D': 'B', 'L': 6, 'R': 8},
        8: {'U': 4, 'D': 'C', 'L': 7, 'R': 9},
        9: {'U': 9, 'D': 9, 'L': 8, 'R': 9},
        'A': {'U': 6, 'D': 'A', 'L': 'A', 'R': 'B'},
        'B': {'U': 7, 'D': 'D', 'L': 'A', 'R': 'C'},
        'C': {'U': 8, 'D': 'C', 'L': 'B', 'R': 'C'},
        'D': {'U': 'B', 'D': 'D', 'L': 'D', 'R': 'D'},
    }

    alt_code = []
    alt_current = 5
    for line in steps:
        for step in line:
            alt_current = alt_keypad[alt_current][step]
        alt_code.append(alt_current)

    alt_code_string = "alt: " + ''.join([str(c) for c in alt_code])

    print alt_code_string


normal()
alt()
